...
namespace 串口demo1
{
    public class DataCheck
    {
        ...
        /// </summary>
        /// <param name="data">要校验的数据数组</param>
        /// <returns>返回LRC校验值</returns>
        public static byte[] DataLrc(byte[] data)
        {
            byte check = 0;
            for (int i = 0; i < data.Length; i++)
            {
            ...
            check = (byte)(~check + 1); // 取反加1，等同于负数运算
            return new byte[] { check };
        }
        ...
        /// </summary>
        /// <param name="data">要校验的数据数组</param>
        /// <returns>返回原数据+LRC校验值</returns>
        public static byte[] DataLrcFull(byte[] data)
        {
            byte[] check = DataLrc(data);
            List<byte> newdata = new List<byte>();
            newdata.AddRange(data);
            newdata.AddRange(check);
            return newdata.ToArray();
